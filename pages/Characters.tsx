  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      <div className="text-center mb-16">
        <h1 className="text-4xl md:text-5xl font-heading font-bold text-white mb-6">{t.characters.title}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {CHARACTERS.map((char) => {
          const transChar = t.characters.list[char.id as keyof typeof t.characters.list];
          const transRole = t.characters.roles[char.role as keyof typeof t.characters.roles] || char.role;
          ...
          return (
            <div key={char.id} className="group flex flex-col bg-[#1a1a1a] rounded-xl border border-white/5 p-6 hover:border-bb-blue/50 transition-all duration-300 hover:shadow-lg hover:shadow-bb-blue/10">
              ...
              <p className="text-gray-400 text-sm leading-relaxed flex-1 mb-8">
                {transChar?.description || char.description}
              </p>
              ...
              <div className="pt-4 border-t border-white/5">
                <button className="text-sm font-bold text-white hover:text-bb-blue transition-colors flex items-center gap-2 group-hover:translate-x-1 duration-200">
                  {t.characters.readFullProfile} <ArrowRight className="w-4 h-4" />
                </button>
              </div>
            </div>
          );
